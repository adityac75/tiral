...
void imuInit(void)
{
    ...
    gyroScaleRad = gyro.scale * (3.14159265358979323846f / 180.0f) * 0.000001f;
}
...
void imuCalculateAcceleration(uint32_t deltaT)
{
    ...
    rpy.angles.yaw = -(float)heading * (3.14159265358979323846f / 180.0f);
...
int16_t imuCalculateHeading(t_fp_vector *vec)
{
    ...
    float hd = (atan2f(Yh, Xh) * 1800.0f / 3.14159265358979323846f + magneticDeclination) / 10.0f;
    head = lrintf(hd);
...
static void imuCalculateEstimatedAttitude(void)
{
    ...
    inclination.values.rollDeciDegrees = lrintf(anglerad[AI_ROLL] * (1800.0f / 3.14159265358979323846f));
    inclination.values.pitchDeciDegrees = lrintf(anglerad[AI_PITCH] * (1800.0f / 3.14159265358979323846f));
