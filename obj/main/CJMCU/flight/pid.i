...
# 6 "c:\\users\\chalpathi\\desktop\\arm\\arm-none-eabi\\include\\math.h" 2 3
# 1 "c:\\users\\chalpathi\\desktop\\arm\\arm-none-eabi\\include\\machine\\ieeefp.h" 1 3
# 7 "c:\\users\\chalpathi\\desktop\\arm\\arm-none-eabi\\include\\math.h" 2 3
# 1 "c:\\users\\chalpathi\\desktop\\arm\\arm-none-eabi\\include\\_ansi.h" 1 3
# 8 "c:\\users\\chalpathi\\desktop\\arm\\arm-none-eabi\\include\\math.h" 2 3
...
pidControllerFuncPtr pid_controller = pidMultiWii;
        ...
        uint16_t max_angle_inclination, rollAndPitchTrims_t *angleTrim, rxConfig_t *rxConfig)
{
    ...
    prop = ((((((rcCommand[PITCH]) > 0 ? (rcCommand[PITCH]) : -(rcCommand[PITCH]))) > (((rcCommand[ROLL]) > 0 ? (rcCommand[ROLL]) : -(rcCommand[ROLL]))) ? (((rcCommand[PITCH]) > 0 ? (rcCommand[PITCH]) : -(rcCommand[PITCH]))) : (((rcCommand[ROLL]) > 0 ? (rcCommand[ROLL]) : -(rcCommand[ROLL]))))) < (500) ? (((((rcCommand[PITCH]) > 0 ? (rcCommand[PITCH]) : -(rcCommand[PITCH]))) > (((rcCommand[ROLL]) > 0 ? (rcCommand[ROLL]) : -(rcCommand[ROLL]))) ? (((rcCommand[PITCH]) > 0 ? (rcCommand[PITCH]) : -(rcCommand[PITCH]))) : (((rcCommand[ROLL]) > 0 ? (rcCommand[ROLL]) : -(rcCommand[ROLL]))))) : (500));

    for (axis = 0; axis < 3; axis++) {
        if (((flightModeFlags & (ANGLE_MODE)) || (flightModeFlags & (HORIZON_MODE))) && (axis == FD_ROLL || axis == FD_PITCH)) {
            ...
            PTermACC = errorAngle * pidProfile->P8[PIDLEVEL] / 100;
        ...
        if (!(flightModeFlags & (ANGLE_MODE)) || (flightModeFlags & (HORIZON_MODE)) || axis == FD_YAW) {
            ...
            if ((((gyroADC[axis]) > 0 ? (gyroADC[axis]) : -(gyroADC[axis])) > (640 * 4)) || (axis == FD_YAW && ((rcCommand[axis]) > 0 ? (rcCommand[axis]) : -(rcCommand[axis])) > 100))
                errorGyroI[axis] = 0;

            ITermGYRO = (errorGyroI[axis] / 125 * pidProfile->I8[axis]) / 64;
        }
        if ((flightModeFlags & (HORIZON_MODE)) && (axis == FD_ROLL || axis == FD_PITCH)) {
            PTerm = (PTermACC * (500 - prop) + PTermGYRO * prop) / 500;
            ITerm = (ITermACC * (500 - prop) + ITermGYRO * prop) / 500;
        } else {
            if ((flightModeFlags & (ANGLE_MODE)) && (axis == FD_ROLL || axis == FD_PITCH)) {
                PTerm = PTermACC;
                ITerm = ITermACC;
            } else {
                PTerm = PTermGYRO;
        ...
        PTerm -= ((int32_t)gyroADC[axis] / 4) * dynP8[axis] / 10 / 8;
            ...
            rollAndPitchTrims_t *angleTrim, rxConfig_t *rxConfig)
{
    ...
    for (axis = 0; axis < 2; axis++) {
        ...
        PTerm -= ((int32_t)(gyroADC[axis] / 4) * dynP8[axis]) >> 6;
...
rollAndPitchTrims_t *angleTrim, rxConfig_t *rxConfig)
{
    ...
    if (pidProfile->dterm_cut_hz) {
        MainDptCut = 0.159154943092f / constrain(pidProfile->dterm_cut_hz, 1, 50);
    } else {
        MainDptCut = 0.159154943092f / 12.0f;
    }
    FLOATcycleTime = (float)constrain(cycleTime, 1, 100000);
    ACCDeltaTimeINS = FLOATcycleTime * 0.000001f;
    RCfactor = ACCDeltaTimeINS / (MainDptCut + ACCDeltaTimeINS);

    if ((flightModeFlags & (HORIZON_MODE))) {
        prop = (float)((((((rcCommand[PITCH]) > 0 ? (rcCommand[PITCH]) : -(rcCommand[PITCH]))) > (((rcCommand[ROLL]) > 0 ? (rcCommand[ROLL]) : -(rcCommand[ROLL]))) ? (((rcCommand[PITCH]) > 0 ? (rcCommand[PITCH]) : -(rcCommand[PITCH]))) : (((rcCommand[ROLL]) > 0 ? (rcCommand[ROLL]) : -(rcCommand[ROLL]))))) < (450) ? (((((rcCommand[PITCH]) > 0 ? (rcCommand[PITCH]) : -(rcCommand[PITCH]))) > (((rcCommand[ROLL]) > 0 ? (rcCommand[ROLL]) : -(rcCommand[ROLL]))) ? (((rcCommand[PITCH]) > 0 ? (rcCommand[PITCH]) : -(rcCommand[PITCH]))) : (((rcCommand[ROLL]) > 0 ? (rcCommand[ROLL]) : -(rcCommand[ROLL]))))) : (450)) / 450.0f;
    }

    for (axis = 0; axis < 2; axis++) {
        int32_t tmp = (int32_t)((float)gyroADC[axis] * 0.3125f);
        gyroADCQuant = (float)tmp * 3.2f;
        rcCommandAxis = (float)rcCommand[axis];
        if ((flightModeFlags & (ANGLE_MODE)) || (flightModeFlags & (HORIZON_MODE))) {
        ...
        if (!(flightModeFlags & (ANGLE_MODE))) {
            if ((((int16_t)gyroADC[axis]) > 0 ? ((int16_t)gyroADC[axis]) : -((int16_t)gyroADC[axis])) > 2560) {
        ...
        axisPID[axis] = lrintf(PTerm + ITerm - DTerm);
# 663 "./src/main/flight/pid.c"
    }

    Mwii3msTimescale = (int32_t)FLOATcycleTime & (int32_t)~3;
    Mwii3msTimescale /= 3000.0f;

    if (pidProfile->pid5_oldyw) {
        PTerm = ((int32_t)pidProfile->P8[FD_YAW] * (100 - (int32_t)controlRateConfig->rates[FD_YAW] * (int32_t)((rcCommand[FD_YAW]) > 0 ? (rcCommand[FD_YAW]) : -(rcCommand[FD_YAW])) / 500)) / 100;
        int32_t tmp = lrintf(gyroADC[FD_YAW] * 0.25f);
        PTerm = rcCommand[FD_YAW] - tmp * PTerm / 80;
        if ((((tmp) > 0 ? (tmp) : -(tmp)) > 640) || (((rcCommand[FD_YAW]) > 0 ? (rcCommand[FD_YAW]) : -(rcCommand[FD_YAW])) > 100)) {
            errorGyroI[FD_YAW] = 0;
        } else {
            error = ((int32_t)rcCommand[FD_YAW] * 80 / (int32_t)pidProfile->P8[FD_YAW]) - tmp;
            errorGyroI[FD_YAW] = constrain(errorGyroI[FD_YAW] + (int32_t)(error * Mwii3msTimescale), -16000, +16000);
        ...
        error = tmp - lrintf(gyroADC[FD_YAW] * 0.25f);
        ...
        ITerm = constrain(errorGyroI[FD_YAW] >> 13, -256, +256);
    ...
    axisPID[FD_YAW] = PTerm + ITerm;
    axisPID[FD_YAW] = lrintf(axisPID[FD_YAW]);
# 716 "./src/main/flight/pid.c"
}
